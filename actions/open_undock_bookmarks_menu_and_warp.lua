...
function M.main(args)
    local bookmark, why = bookmarks.find("undock")
    if bookmark == nil then
        log.error("flight", "the undock bookmark cannot be used: " .. tostring(why))
        return "Failure"
    end

    local entry, err = pointer.open_menu_and_choose(bookmark.region, warp_choice.choices)
    if entry == nil then
        log.error("flight", "the undock bookmark gave no menu: " .. tostring(err))
        return "Failure"
    end
